...
@section('content')
    ...
    <section class="py-12 bg-white min-h-screen">
        <div class="max-w-7xl mx-auto px-6">
            ...
            @if(session('status'))
                <div class="bg-gray-50 border-l-4 border-green-500 text-gray-600 px-4 py-3 mb-8 text-sm flex items-center shadow-sm">
                    <i class="fas fa-check-circle text-green-500 mr-2"></i> {{ session('status') }}
                </div>
            @endif
            @if(session('error'))
                <div class="bg-gray-50 border-l-4 border-red-500 text-gray-600 px-4 py-3 mb-8 text-sm flex items-center shadow-sm">
            ...
            @if($keranjangItems->count() > 0)
                <div class="flex flex-col lg:flex-row gap-12 lg:gap-16">
                    ...
                    <div class="lg:w-2/3">
                        <div class="flex flex-col">
                            @foreach($keranjangItems as $item)
                                <div class="flex gap-4 md:gap-6 py-6 border-b border-gray-100 last:border-0 group">
                                    ...
                                    {{-- Product Image --}}
                                    <div class="w-24 h-32 md:w-32 md:h-40 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden border border-gray-100 relative">
                                        <a href="{{ route('produk.show', $item->produk->id) }}">
                                            @if($item->produk->image)
                                                <img src="{{ asset('storage/' . $item->produk->image) }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
                                            @else
                                    ...
                                    <div class="flex flex-col flex-grow justify-between">
                                        ...
                                        <div class="flex justify-between items-start">
                                            <div>
                                                ...
                                                <h3 class="text-base md:text-lg font-bold text-black leading-tight">
                                                    <a href="{{ route('produk.show', $item->produk->id) }}" class="hover:underline">
                                                        {{ $item->produk->name }}
                                                    </a>
                                                </h3>
                                            </div>
                                            ...
                                            <form action="{{ route('keranjang.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-gray-400 hover:text-red-500 transition p-1" title="Remove Item">
                                        ...
                                        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-4">
                                            ...
                                            {{-- Qty Control --}}
                                            <form action="{{ route('keranjang.update', $item->id) }}" method="POST" class="flex items-center gap-3">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <div class="flex items-center border border-gray-300 rounded-md h-10 w-32">
                                                    ...
                                                    <input type="number" name="jumlah" value="{{ $item->jumlah }}" min="1" max="{{ $item->produk->stock }}" 
                                                ...
                                                <button type="submit" class="text-xs font-bold text-black border-b border-black pb-0.5 hover:text-gray-600 hover:border-gray-600 transition md:hidden">
                                                    Update
                                                </button>
                                            </form>
                        ...
                        <div class="mt-8">
                            <a href="{{ route('home') }}" class="inline-flex items-center text-sm font-bold text-black hover:text-gray-600 transition group">
                    ...
                    <div class="lg:w-1/3">
                        <div class="bg-gray-50 rounded-xl p-6 lg:p-8 sticky top-8 border border-gray-100">
                            <h2 class="text-xl font-heading font-bold text-black uppercase mb-6 tracking-wide">Order Summary</h2>
                            ...
                            <div class="border-t border-gray-200 pt-4 mb-8">
                                <div class="flex justify-between items-end">
                                    ...
                                    <span class="text-2xl font-bold text-black">Rp {{ number_format($total, 0, ',', '.') }}</span>
                                </div>
                            </div>
            ...
            @else
                {{-- EMPTY STATE --}}
                <div class="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                    <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center mb-6 shadow-sm text-gray-300">
                    ...
                    <p class="text-gray-500 mb-8 max-w-sm mx-auto">Looks like you haven't added anything to your cart yet.</p>
                    <a href="{{ route('home') }}" class="px-8 py-3 bg-black text-white text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-gray-800 transition">
                        Start Shopping
                    </a>
                </div>
            @endif
