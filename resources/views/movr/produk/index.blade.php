        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="md:w-1/4">
                <div class="bg-card-bg border border-border-color rounded-lg p-6 sticky top-6">
                    <h3 class="text-lg font-bold text-light-text mb-4 border-b border-border-color pb-2">Filter</h3>
                    
                    <div class="mb-6">
                        <h4 class="font-medium text-light-text mb-3">Kategori</h4>
                        <ul class="space-y-2">
                            <li>
                                <a href="{{ route('produk.index') }}" class="text-gray-400 hover:text-accent-green transition {{ !request('kategori') ? 'text-accent-green font-bold' : '' }}">
                                    Semua Kategori
                                </a>
                            </li>
                            @foreach($categories as $cat)
                            <li>
                                <a href="{{ route('produk.index', ['kategori' => $cat->slug]) }}" class="text-gray-400 hover:text-accent-green transition {{ request('kategori') == $cat->slug ? 'text-accent-green font-bold' : '' }}">
                                    {{ $cat->name }}
                                </a>
                            </li>
                            @endforeach
            ...
            <div class="md:w-3/4">
                <div class="flex justify-between items-center mb-6">
                    <p class="text-gray-400">{{ $products->total() }} produk ditemukan</p>
                </div>
                            ...
                            <div class="group bg-card-bg rounded-xl border border-border-color overflow-hidden hover:border-accent-green transition-all duration-300 lift-effect">
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="mt-4 flex items-center gap-2">
                                        ...
                                        <a href="{{ route('produk.show', $item->id) }}" class="bg-dark-bg border border-border-color text-light-text p-2 rounded-lg hover:border-blue-400 hover:text-blue-400 transition h-full flex items-center justify-center" title="Detail">
                ...
                @else
                    <div class="text-center py-16 bg-card-bg rounded-xl border border-border-color">
                        ...
                        <p class="text-gray-400">Belum ada produk atau coba ganti filter kategori.</p>
                    </div>
                @endif
            </div>
        </div>
